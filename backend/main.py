...
"""
SoraDirector Backend - Main Application

本文件状态: 渐进式重构中 (阶段7已完成)
备份文件: main_legacy.py (完整备份，2025-12-27)

⚠️ 重要说明:
1. 本文件包含新旧两套架构，当前处于共存阶段
2. 新架构已全部完成，旧代码保留作为备份和参考
3. 生产环境需要运行2周后，再考虑清理旧代码

🎉 新架构模块 (已完成):
✅ routers/health.py        - 健康检查 (1个接口)
✅ routers/user.py          - 用户管理 (3个接口)
✅ routers/admin.py         - 管理员管理 (12个接口)
✅ routers/video.py         - 视频管理 (5个接口)
✅ routers/product.py       - 商品管理 (5个接口)
✅ routers/prompt.py        - 提示词管理 (3个接口)
✅ routers/character.py     - 角色管理 (1个接口)
✅ routers/project.py       - 项目管理 (2个接口)
✅ routers/image.py         - 图片处理 (5个接口)
✅ routers/ai_chat.py       - AI聊天和脚本 (3个接口)
✅ routers/ai_generation.py - AI生成服务 (5个接口)

📦 服务模块:
✅ services/ai_helper.py    - AI辅助服务 (chat_with_ai, generate_video_with_ai)
✅ services/tos_service.py  - TOS对象存储服务
✅ services/credit_service.py - 积分管理服务
✅ services/ai_service.py   - AI服务封装

🗑️ 旧代码标记 (待清理):
⚠️ 以下区域的代码已迁移到新模块，保留供参考:

1. 线 200-450:   Pydantic数据模型 (→ 已移至各router模块)
2. 线 450-700:   AI工具函数 (→ services/ai_helper.py)
3. 线 700-2000:  旧路由定义 (→ routers/*)
4. 线 2000-3200: 微信支付接口 (保留)

📅 清理计划:
- 阶段1 (2025-12-27): ✅ 创建备份 main_legacy.py
- 阶段2 (1周后):   ⏳ 注释旧路由定义
- 阶段3 (2周后):   ⏳ 删除重复数据模型
- 阶段4 (1个月后): ⏳ 最终精简至300行
