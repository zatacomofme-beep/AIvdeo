...
export function CharacterSelector({ onSelectCharacter, selectedCharacter }: CharacterSelectorProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 自动选中新创建的角色
      setTimeout(() => {
        const newestCharacter = myCharacters[0];
        if (newestCharacter) {
          onSelectCharacter(newestCharacter);
        }
      }, 100);
    ...
    <div className="space-y-6 relative z-10 animate-in slide-in-from-right-10 duration-500">
      ...
      {myCharacters.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 px-4">
          ...
          <p className="text-lg text-slate-600 mb-2">暂无角色</p>
        ...
        <>
          ...
          {selectedCharacter && (
            <button
              ...
              className="w-full px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg transition-all text-sm"
            >
              取消选择
            </button>
          )}
